...
//    $totalPost = mysqli_query($conn, "SELECT * FROM blog_posts, users WHERE blog_posts.user_id = users.user_id");
    ...
    <h2 class="list-post-heading">Danh sách bài viết: <span><?php $rowFullname = mysqli_fetch_array($sqlFullname);
            echo $rowFullname['fullname'] ?></span></h2>
    <div class="content">
        <!--Post-->
        <div class="posts">
            <?php
                ...
                if ($_SESSION['user_id'] == $rowList['user_id']) { ?>
                    <div class="post-items">
                        <a href="./post_detail.php?post_id=<?php echo $rowList['post_id'] ?>"><img
                                    src="./img/img_post/<?php echo $rowList['image_url'] ?>" alt=""
                                    class="post-items-img"></a>
                        <div>
                            <p class="post-items-title"><a
                                        href="./post_detail.php?post_id=<?php echo $rowList['post_id'] ?>"><?php echo $rowList['title'] ?></a>
                            </p>
                            ...
                            <p class="post-items-description">
                                ...
                                <a href="./post_detail.php?post_id=<?php echo $rowList['post_id'] ?>"
                                   href="post_detail.php?post_id=<?php echo $rowList['post_id'] ?>"> ...xem thêm</a>
                            </p>
                        </div>
                    </div>
